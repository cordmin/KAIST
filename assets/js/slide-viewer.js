...
// =========================================================================
const oledSlideData = [
  { tag: "1차시 탐구 · [9과10-03]", title: "자외선 형광 탐정단: 일상 속 숨은 빛의 발견", body: "• 토닉워터(퀴닌): 파란색 형광 발광<br>• 비타민(리보플라빈): 연두색 형광 발광<br>• 올리브유(엽록소): 붉은색 형광 발광<br>• 천원권 지폐: 주황색 형광 섬유 발광<br>→ 보이지 않는 자외선을 흡수하여 가시광선 빛을 방출하는 현상" },
  { tag: "빛의 스펙트럼", title: "빛의 파장과 에너지: 파장이 짧을수록 높은 에너지", body: "• 자외선(300~400nm): 에너지가 큼<br>• 가시광선(400~700nm): 인간의 눈으로 볼 수 있는 색채 영역<br>→ 높은 에너지를 흡수하여 낮은 에너지를 방출할 때, 차이나는 에너지는 어디로 갈까?" },
  ...
  { tag: "[9과14-02] OLED 해결 과제", title: "스핀 통계 한계: 스마트폰 OLED의 75% 열 손실", body: "• 배터리 전기를 빛을 내는 분자에 주입할 때의 양자 스핀 통계 확률<br>• 25%만 단일항이 되어 유용한 빛을 방출<br>• 75%는 삼중항에 갇혀 빛 대신 디스플레이 발열 유발<br>→ 배터리 소모와 발열, 패널 수명 단축의 근본 원인" },
  ...
  { tag: "프로젝트 탐구", title: "차세대 청색 OLED 기술 탐구 프로젝트", body: "• 파란색 OLED 해결 과제: 높은 빛 에너지와 열 스트레스로 인한 분자 결합 손상(번인)<br>• 핵심 원리: 불필요한 열 진동을 억제하여 버려지던 75% 에너지를 100% 빛으로 초고속 전환<br>• 기대 효과: 발열 차단으로 배터리·방열판 무게를 줄인 초경량 스마트폰 및 방열판 없는 롤러블 화면 구현" },
